...
def download_tickers(download_data_fn, tickers, start):
    ...
    with futures.ThreadPoolExecutor() as executor:
        ...
        for ticker in tickers:
            _futures.append(
                executor.submit(download_data_fn, ticker=ticker, start_epoch=start_epoch, end_epoch=end_epoch)
            )
...
def download_data(db_dir, download_data_fn, recreate = False):
    if recreate:
